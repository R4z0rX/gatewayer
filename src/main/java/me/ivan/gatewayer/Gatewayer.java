...
public class Gatewayer {
    ...
    public void renderTexts(MatrixStack stack, float tickDelta) {
        ...
        if (mc.player.getWorld().getRegistryKey().equals(World.END)) {
            ...
            if (gatewayInfo.isEnabled() && camera.getPos().squaredDistanceTo(new Vec3d(gatewayInfo.getPos().getX(), gatewayInfo.getPos().getY(), gatewayInfo.getPos().getZ())) < MAX_RENDER_DISTANCE * MAX_RENDER_DISTANCE) {
                ...
                drawString(gatewayInfo.getEntityExitPos(), tickDelta, 0F, new String[]{Text.translatable("gatewayer.entity_exit_position", gatewayInfo.getEntityExitPos().toShortString()).getString()}, new int[]{Formatting.BLUE.getColorValue()});
                drawString(gatewayInfo.getPos(), tickDelta, currentLine++, new String[]{Text.translatable("gatewayer.gateway_position", gatewayInfo.getPos().toShortString()).getString()}, new int[]{Formatting.GOLD.getColorValue()});
                ...
                if (gatewayInfo.hasBlockEntity()) {
                    ...
                    if (tag.contains("ExitPortal")) {
                        drawString(gatewayInfo.getPos(), tickDelta, currentLine++, new String[]{Text.translatable("gatewayer.exit_portal", tag.get("ExitPortal").toString()).getString().replace("{", "").replace("}", "")}, new int[]{Formatting.DARK_GREEN.getColorValue()});
                    } else {
                        drawString(gatewayInfo.getPos(), tickDelta, currentLine++, new String[]{Text.translatable("gatewayer.no_exit_portal").getString()}, new int[]{Formatting.RED.getColorValue()});
                    }
                    drawString(gatewayInfo.getPos(), tickDelta, currentLine++, new String[]{Text.translatable(gateway.needsCooldownBeforeTeleporting() ? "gatewayer.cooling_down" : "gatewayer.ready_to_teleport").getString()}, new int[]{gateway.needsCooldownBeforeTeleporting() ? Formatting.RED.getColorValue() : Formatting.DARK_GREEN.getColorValue()});

                    if (tag.contains("ExactTeleport") && tag.get("ExactTeleport").toString().equals("1b")) {
                        drawString(gatewayInfo.getPos(), tickDelta, currentLine++, new String[]{Text.translatable("gatewayer.exact_teleport").getString()}, new int[]{Formatting.YELLOW.getColorValue()});
                    }
                } else {
                    drawString(gatewayInfo.getPos(), tickDelta, currentLine++, new String[]{Text.translatable("gatewayer.non_block_entity").getString()}, new int[]{Formatting.RED.getColorValue()});
                }
            }
        }
    }
    ...
    public static void drawString(BlockPos pos, float tickDelta, float line, String[] texts, int[] colors) {
        ...
        if (camera.isReady() && client.getEntityRenderDispatcher().gameOptions != null && client.player != null) {
            ...
            for (int i = 0; i < texts.length; i++) {
                ...
                client.textRenderer.draw(texts[i], renderX, renderY, colors[i], false, matrix4f, immediate, TextRenderer.TextLayerType.SEE_THROUGH, 0, 0xF000F0);
                immediate.draw();
